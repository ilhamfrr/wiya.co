...
<div class="max-w-4xl mx-auto animate-fade-in">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Product</h2>
            <p class="text-gray-500 mt-1">Update product details and availability.</p>
        </div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="{ preview: '{{ $product->image ? asset('storage/' . $product->image) : null }}' }">
        @csrf
        @method('PUT')
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $product->name }}" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 text-lg font-bold focus:ring-2 focus:ring-black transition-all" required>
                </div>
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="price" value="{{ $product->price }}" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 text-lg font-bold focus:ring-2 focus:ring-black" required>
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Stock</label>
                    <input type="number" name="stock" value="{{ $product->stock }}" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black" required>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Description</label>
                    <textarea name="description" rows="5" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black resize-none">{{ $product->description }}</textarea>
                </div>
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Colors</label>
                        <input type="text" name="colors" value="{{ is_array($product->colors) ? implode(', ', $product->colors) : '' }}" placeholder="e.g. Sage, Terracotta" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black">
                    ...
                    <div>
                        ...
                        <input type="text" name="sizes" value="{{ is_array($product->sizes) ? implode(', ', $product->sizes) : '' }}" placeholder="e.g. S, M, L, XL" class="w-full bg-gray-50 border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-black">
                        <p class="text-[9px] text-gray-400 mt-2 italic font-bold">Separated by commas</p>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <button type="submit" class="w-full bg-black text-white font-black py-5 rounded-[2rem] shadow-xl hover:bg-gray-900 transition-all active:scale-[0.98]">Update Product</button>
        </div>
    </form>
</div>
@endsection
